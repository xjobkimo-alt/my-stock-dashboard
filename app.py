...
if "watchlist_dict" not in st.session_state:
    if os.path.exists(SAVE_FILE):
        ...
        st.session_state["watchlist_dict"] = { 
            "加權指數": "^TWII", "台積電 (2330)": "2330.TW", "鴻海 (2317)": "2317.TW", "聯發科 (2454)": "2454.TW" 
        }
...
TAIWAN_STOCK_DICT = {
    "2330": "台積電", "2317": "鴻海", "2454": "聯發科", "2882": "國泰金", "2881": "富邦金", "2303": "聯電", 
    "2603": "長榮", "2609": "陽明", "2615": "萬海", "2002": "中鋼", "2412": "中華電", "2308": "台達電",
    "2891": "中信金", "2886": "兆豐金", "2884": "玉山金", "2892": "第一金", "2890": "永豐金"
}
...
def fetch_safe_stock_data(ticker): 
    ...
    return stock.history(period="5y"), stock.info 
...
def get_ai_analysis(stock_name, price, change, pct, ma5, k_val, d_val): 
    try: 
        ...
        prompt = f"分析股票走勢：{stock_name}，當前價格: {price}，今日漲跌: {change} ({pct}%)，MA5: {ma5}，請給予繁體中文短評並提供策略建議。" 
        ...
        if new_code: 
            ...
            try:
                ...
                if test_stock.history(period="1d").empty:
                    ...
                    st.rerun()
            except: st.sidebar.error("❌ 無法連線驗證。")
...
if st.sidebar.button("❌ 從清單中刪除目前股票"): 
    if len(st.session_state["watchlist_dict"]) > 1: 
        ...
        st.session_state["current_selected_idx"] = 0
        st.rerun() 
    else: st.sidebar.warning("清單內至少需保留一檔股票！") 
...
with row1_col1:
    ...
    h1, h2, h3, h4 = st.columns([2, 1.2, 1, 1.2])
    h1.markdown("<p style='text-align:center; font-weight:bold; color:#888; font-size:13px;'>商品名稱</p>", unsafe_allow_html=True)
    h2.markdown("<p style='text-align:center; font-weight:bold; color:#888; font-size:13px;'>成交價</p>", unsafe_allow_html=True)
    h3.markdown("<p style='text-align:center; font-weight:bold; color:#888; font-size:13px;'>漲跌</p>", unsafe_allow_html=True)
    h4.markdown("<p style='text-align:center; font-weight:bold; color:#888; font-size:13px;'>漲幅(%)</p>", unsafe_allow_html=True)
    ...
    for idx, (name, code) in enumerate(st.session_state["watchlist_dict"].items()):
        try:
            ...
            chg = c_p - s_info.get("previousClose", s_df['Close'].iloc[-2])
            pct = (chg / s_info.get("previousClose", s_df['Close'].iloc[-2])) * 100
        except: c_p, chg, pct = 0.0, 0.0, 0.0
            
        css_class = "stock-up" if chg > 0 else ("stock-down" if chg < 0 else "")
        b_sign = "+" if chg >= 0 else ""
        b1, b2, b3, b4 = st.columns([2, 1.2, 1, 1.2])
        with b1:
            if st.button(f"📌 {name}", key=f"btn_{code}_{idx}", use_container_width=True):
                st.session_state["current_selected_idx"] = watchlist_keys.index(name); st.rerun()
        b2.markdown(f"<p style='text-align:center; padding-top:6px; font-family:monospace; font-size:13px;'>{c_p:,.2f}</p>", unsafe_allow_html=True)
        b3.markdown(f"<p style='text-align:center; padding-top:6px; font-family:monospace; font-size:13px;' class='{css_class}'>{b_sign}{chg:,.2f}</p>", unsafe_allow_html=True)
        b4.markdown(f"<p style='text-align:center; padding-top:6px; font-family:monospace; font-size:13px;' class='{css_class}'>{b_sign}{pct:.2f}%</p>", unsafe_allow_html=True)
